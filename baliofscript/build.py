...
SITE_URL = "https://baliofscript.com"
...
def nav(active=""):
    links = "".join(
        f'<a href="/{k}/"{" class=\'on\'" if k == active else ""}>{v[0]}</a>'
        for k, v in CATEGORIES.items()
    )
    return f"""<header class="masthead">
<div class="wrap masthead-inner">
<a class="wordmark" href="/"><span>Bali</span> Off Script</a>
<button class="menu-btn" aria-label="Menu" aria-expanded="false">Menu</button>
<nav class="nav">{links}<a class="nav-search" href="/search/">Search</a></nav>
</div>
</header>"""
...
<a href="/checklist/">Due diligence checklist</a>
...
def article(m, siblings):
    ...
    related = "".join(
        f'<li><a href="/{s["category"]}/{s["slug"]}/">{s["question"]}</a></li>'
...
<p class="eyebrow"><a href="/{m['category']}/">{cat_name}</a></p>
<h1>{m["question"]}</h1>
...
<span class="ledger-n">{len([p for p in pages if p["category"] == k])}</span>
    ...
    recent = "".join(
        f"""<li class="card"><a href="/{p['category']}/{p['slug']}/">
...
def main():
    ...
    open(os.path.join(OUT, "CNAME"), "w").write("baliofscript.com\n")
